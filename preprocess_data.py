...
def read_train_data(IMG_WIDTH=256, IMG_HEIGHT=256, IMG_CHANNELS=3):
    ...
    if os.path.isfile("data/train_img.npy") and os.path.isfile("data/train_mask.npy"):
        ...
        X_train = np.load("data/train_img.npy")
        Y_train = np.load("data/train_mask.npy")
    ...
    for n, id_ in tqdm(enumerate(train_ids), total=len(train_ids)):
        ...
        Y_train[n] = mask
    np.save("data/train_img", X_train)
    np.save("data/train_mask", Y_train)
    return X_train, Y_train
...
def read_test_data(IMG_WIDTH=256, IMG_HEIGHT=256, IMG_CHANNELS=3):
    ...
    for n, id_ in tqdm(enumerate(test_ids), total=len(test_ids)):
        ...
        img = resize(img, (IMG_HEIGHT, IMG_WIDTH), mode='constant', preserve_range=True)
        X_test[n] = img
    np.save("data/test_img", X_test)
    np.save("data/test_size", sizes_test)
    return X_test, sizes_test
